#!/usr/bin/env bash
# install.sh — Yaklog interactive installer (Task #284 / PLAN-YAKLOG-
...
log() { printf '\033[36m[yaklog-install]\033[0m %s\n' "$*"; }
err() { printf '\033[31m[yaklog-install]\033[0m %s\n' "$*" >&2; }
ok()  { printf '\033[32m[yaklog-install]\033[0m %s\n' "$*"; }
...
if [[ "$INSTALL_DIR" == "/opt/yaklog" ]]; then
  INSTALL_DIR="/opt/$INSTANCE_NAME"
fi
...
YAKLOG_ADMIN_BEARER="yaklog_$(openssl rand -hex 32)"
...
export YAKLOG_ADMIN_BEARER INSTANCE_NAME YAKLOG_BIND_IP YAKLOG_BIND_IP \
...
cat > "$INSTALL_DIR/yaklog-grafana.env" <<EOF
GF_SECURITY_ADMIN_USER=admin
GF_SECURITY_ADMIN_PASSWORD=${GRAFANA_ADMIN_PW}
EOF
chmod 600 "$INSTALL_DIR/yaklog-grafana.env"

cat > "$INSTALL_DIR/yaklog-minio.env" <<EOF
MINIO_ROOT_USER=yaklog-admin
MINIO_ROOT_PASSWORD=${MINIO_ROOT_PW}
EOF
chmod 600 "$INSTALL_DIR/yaklog-minio.env"

ok 'Config materialized (mode 600 on secrets).'

# ── 8. Bring up the stack ────────────────────────────────────────────────

log "Bringing up Yaklog stack…"
...
if [[ "$(id -u)" == "0" || -n "${SUDO_UID:-}" ]] && command -v systemctl >/dev/null; then
  ...
  ETC_DIR="/etc/yaklog-${INSTANCE_NAME}"
  TEXTFILE_DIR="/var/lib/yaklog-${INSTANCE_NAME}/textfile/output-ingester"
  OPS_USER="root"
  ...
  printf '%s\n' "$YAKLOG_ADMIN_BEARER" > "$OPS_KEY_PATH"
...
if [[ -n "$TRACK_GITHUB_REPO" ]]; then
  ...
  curl -sf -X POST -H "Authorization: Bearer $YAKLOG_ADMIN_BEARER" \
...
═══════════════════════════════════════════════════════════════════════
  Yaklog '${INSTANCE_NAME}' installation complete
═══════════════════════════════════════════════════════════════════════
  ...
  MinIO:      http://127.0.0.1:${MINIO_CONSOLE_PORT}/       (user=yaklog-admin  pass=${MINIO_ROOT_PW})
              (127.0.0.1-bound; SSH-forward to reach)

  ── yaklog-admin bearer (SAVE THIS NOW; not shown again) ──
  ${YAKLOG_ADMIN_BEARER}

  Use it as:
    Authorization: Bearer ${YAKLOG_ADMIN_BEARER}
  ...
  Files (mode 600 — DO NOT commit):
    ${INSTALL_DIR}/.env
    ${INSTALL_DIR}/yaklog-grafana.env
    ${INSTALL_DIR}/yaklog-minio.env
