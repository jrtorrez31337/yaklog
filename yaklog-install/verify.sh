#!/usr/bin/env bash
# verify.sh — post-install sanity for a Yaklog instance.
# Task #284 / PLAN-YAKLOG-INSTALL-BUNDLE.md §5.
#
...
YAKLOG_ADMIN_BEARER="${YAKLOG_OPS_API_KEYS%%,*}"
...
echo
echo "=== Yaklog '$INSTANCE_NAME' verify (${INSTALL_DIR}) ==="
echo
...
if [[ -n "$YAKLOG_ADMIN_BEARER" ]]; then
  body=$(curl -sf --max-time 3 -H "Authorization: Bearer $YAKLOG_ADMIN_BEARER" \
         "http://127.0.0.1:${DASHBOARD_PORT}/api/v1/presence/public" 2>/dev/null || echo "")
  if [[ -n "$body" ]]; then
    pass "yaklog-admin bearer authenticated + presence returned"
  else
    fail "yaklog-admin bearer failed to authenticate"
  fi
else
...
echo 'Yaklog install verified clean.'
