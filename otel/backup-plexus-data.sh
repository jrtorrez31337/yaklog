#!/usr/bin/env bash
# Plexus Prom + Grafana volume backup -- Stage 1 minimum.
#
# Produces two tarballs in $BACKUP_DIR (default ~/yaklog-backups):
#   plexus-prom-YYYYMMDD-HHMMSS.tar.gz     -- Prom WAL + TSDB blocks
#   plexus-grafana-YYYYMMDD-HHMMSS.tar.gz  -- Grafana sqlite + plugin state
#
...
#
# Usage:
#   ./otel/backup-plexus-data.sh                     # backup both volumes
#   BACKUP_DIR=/somewhere ./otel/backup-plexus-data.sh
#   ./otel/backup-plexus-data.sh --prom-only         # just Prom
#   ./otel/backup-plexus-data.sh --grafana-only      # just Grafana
#   ./otel/backup-plexus-data.sh --verify <tarball>  # integrity test
#
...
backup_volume() {
  local volume="$1"
  local label="$2"
  local out="$BACKUP_DIR/plexus-${label}-${STAMP}.tar.gz"
...
[[ $DO_PROM    == 1 ]] && backup_volume yaklog_plexus_prom_data    prom
[[ $DO_GRAFANA == 1 ]] && backup_volume yaklog_plexus_grafana_data grafana

echo ""
echo "== existing backups in $BACKUP_DIR =="
ls -lh "$BACKUP_DIR"/plexus-*.tar.gz 2>/dev/null | tail -10 || echo "(none)"

# ---------------------------------------------------------------------------
# RESTORE PROCEDURE (manual; rare; data-loss possible -- read carefully)
# ---------------------------------------------------------------------------
# 1. Stop the consumer of the volume:
#      docker compose stop plexus-prometheus   # or plexus-grafana
# 2. Blow away the existing volume data:
#      docker run --rm -v yaklog_plexus_prom_data:/data alpine sh -c 'rm -rf /data/*'
# 3. Restore from tarball:
#      docker run --rm \
#        -v yaklog_plexus_prom_data:/data \
#        -v "$BACKUP_DIR:/backup:ro" \
#        alpine:3.20 \
#        sh -c "cd /data && tar -xzf /backup/plexus-prom-<STAMP>.tar.gz"
# 4. Restart the consumer:
#      docker compose start plexus-prometheus
# 5. Verify:
