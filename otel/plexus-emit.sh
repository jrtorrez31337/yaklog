#!/usr/bin/env bash
# plexus-emit.sh — push one OTLP metric (or log) to the Plexus collector
...
#
# Usage:
#   plexus-emit.sh <metric-name> <kind> <value> [attr=value ...]
#     kind ∈ {counter, gauge}
#     metric-name dots are auto-underscored at Prom export
#     attrs become DATA-POINT attributes (analytical slicing dims)
#
#   plexus-emit.sh --log <severity> <body-text> [attr=value ...]
#     severity ∈ {trace, debug, info, warn, error, fatal}
#
#   plexus-emit.sh --dry-run …    # print payload + curl, don't POST
#
# Resource attributes (agent identity) come from env, set ONCE by the
# calling runtime's launcher:
#   PLEXUS_AGENT_ID       e.g. gemini-agent
#   PLEXUS_CLUSTER_ID     default ssw-dev
#   PLEXUS_DEPLOYMENT     default devel
#   PLEXUS_SERVICE_NAME   e.g. gemini-cli   (free-form; analogous to CC's "claude-code")
#   PLEXUS_OTLP_ENDPOINT  default http://192.168.122.76:4328
#   YAKLOG_TOKEN          reused as OTLP Bearer (no separate credential)
#
# Examples:
#   # counter: gemini hit Google's quota cap
#   plexus-emit.sh gemini.quota_exhausted counter 1 \
#     model=gemini-2.5-pro reset_seconds=13200
#
#   # gauge: how many minutes until quota resets
#   plexus-emit.sh gemini.quota_reset_seconds gauge 13200 model=gemini-2.5-pro
#
#   # log line tied to the same resource attrs
#   plexus-emit.sh --log warn "Gemini CLI exited status 1: QUOTA_EXHAUSTED" \
#     model=gemini-2.5-pro reset_seconds=13200
#
# Exit codes:
#   0  posted (HTTP 2xx)
#   1  bad usage / missing env
#   2  Plexus collector returned non-2xx (body printed to stderr)

set -euo pipefail

PLEXUS_OTLP_ENDPOINT="${PLEXUS_OTLP_ENDPOINT:-http://192.168.122.76:4328}"
PLEXUS_CLUSTER_ID="${PLEXUS_CLUSTER_ID:-ssw-dev}"
PLEXUS_DEPLOYMENT="${PLEXUS_DEPLOYMENT:-devel}"
PLEXUS_SERVICE_NAME="${PLEXUS_SERVICE_NAME:-unknown}"

err() { printf 'plexus-emit: %s\n' "$*" >&2; }

if [[ -z "${PLEXUS_AGENT_ID:-}" ]]; then
  err "PLEXUS_AGENT_ID env var required"
  exit 1
fi
...
build_resource_attrs() {
  cat <<EOF
[
  {"key": "plexus.agent_id",   "value": {"stringValue": "${PLEXUS_AGENT_ID}"}},
  {"key": "plexus.cluster_id", "value": {"stringValue": "${PLEXUS_CLUSTER_ID}"}},
  {"key": "plexus.deployment", "value": {"stringValue": "${PLEXUS_DEPLOYMENT}"}},
  {"key": "service.name",      "value": {"stringValue": "${PLEXUS_SERVICE_NAME}"}}
]
EOF
}
...
post_payload() {
  ...
  local payload="$2"
  if (( DRY_RUN )); then
    echo "=== plexus-emit DRY-RUN ==="
    echo "POST ${PLEXUS_OTLP_ENDPOINT}${path}"
  ...
  http=$(curl -sS -o "$resp" -w '%{http_code}' \
    -X POST "${PLEXUS_OTLP_ENDPOINT}${path}" \
...
if [[ "${1:-}" == "--log" ]]; then
  ...
  BODY_ESC="${BODY_ESC//\"/\\\"}"
  PAYLOAD=$(cat <<EOF
{"resourceLogs":[{"resource":{"attributes":${RES_ATTRS}},"scopeLogs":[{"scope":{"name":"plexus.emit"},"logRecords":[{"timeUnixNano":"${NOW_NS}","observedTimeUnixNano":"${NOW_NS}","severityNumber":${SEV_NUM},"severityText":"$(echo "$SEVERITY" | tr a-z A-Z)","body":{"stringValue":"${BODY_ESC}"},"attributes":${KV_ATTRS}}]}]}]}
EOF
)
...
# ── metrics path ──────────────────────────────────────────────────────
METRIC_NAME="${1:?usage: plexus-emit.sh <metric-name> <counter|gauge> <value> [attr=val ...]}"; shift
...
PAYLOAD=$(cat <<EOF
{"resourceMetrics":[{"resource":{"attributes":${RES_ATTRS}},"scopeMetrics":[{"scope":{"name":"plexus.emit"},"metrics":[{"name":"${METRIC_NAME}",${METRIC_BODY}}]}]}]}
EOF
)
