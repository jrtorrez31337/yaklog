#!/usr/bin/env bash
# install-yaklog-otel-runtime.sh — Codex CLI + Gemini CLI seat OTel onboarding.
# Per ADR-0032 Phase 0 Item A (yaklog-dev PLAN-ADR-0032-PHASE-0-CROSS-RUNTIME-TELEMETRY-PARITY.md §3 Task A.3).
#
# Sister to install-yaklog-otel.sh (which is Claude Code-specific —
...
#
# Usage:
#   bash install-yaklog-otel-runtime.sh --runtime codex             # current $HOME (OVERWRITES)
#   bash install-yaklog-otel-runtime.sh --runtime codex --merge-preserve
#                                                                  # preserves existing
...
#   bash install-yaklog-otel-runtime.sh --runtime gemini            # current $HOME (OVERWRITES)
#   bash install-yaklog-otel-runtime.sh --runtime gemini --merge-preserve
