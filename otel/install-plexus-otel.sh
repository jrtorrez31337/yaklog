#!/usr/bin/env bash
# install-plexus-otel.sh v2 — self-install Plexus OTel opt-in for one CC agent.
#
...
# traptop10k) don't collide on plexus.agent_id.
#
# What this does:
...
#   4. Merges the Plexus OTel env keys into the `env` section. Other env
#      keys + other settings preserved. Re-runs cleanly REPLACE the
#      managed keys (tracked via _plexus_managed_env_keys in the file).
...
#   bash install-plexus-otel.sh <agent-id> --workspace=<path>       # explicit workspace
#   bash install-plexus-otel.sh <agent-id> --dry-run                # show what would happen
#   bash install-plexus-otel.sh <agent-id> --inline-token           # bake literal token
#   bash install-plexus-otel.sh --uninstall <agent-id> [--workspace=<path>]
#
# Pre-conditions:
...
if (( UNINSTALL )); then
  ...
  if (( DRY_RUN )); then
    log "  would strip Plexus-managed env keys from $SETTINGS_FILE"
    exit 0
  fi
  cp "$SETTINGS_FILE" "${SETTINGS_FILE}.bak.pre-plexus-uninstall-$(date +%Y%m%dT%H%M%SZ)"
  python3 - "$SETTINGS_FILE" <<'PYEOF'
import sys, json
p = sys.argv[1]
with open(p) as f: s = json.load(f)
mgr = s.get('_plexus_managed_env_keys') or []
env = s.get('env') or {}
for k in mgr: env.pop(k, None)
s.pop('_plexus_managed_env_keys', None)
if not env: s.pop('env', None)
else:       s['env'] = env
with open(p, 'w') as f: json.dump(s, f, indent=2); f.write('\n')
PYEOF
  log "  stripped Plexus env keys from $SETTINGS_FILE"
  exit 0
fi
...
declare -A OTEL_VALS=(
  ...
  [OTEL_RESOURCE_ATTRIBUTES]="plexus.agent_id=${AGENT_ID},plexus.cluster_id=ssw-dev,plexus.deployment=devel"
...
# Replace Plexus-managed keys; preserve any other keys the user has set
for k in keys:
    env[k] = os.environ[f'PX_VAL__{k}']

s['env'] = env
s['_plexus_managed_env_keys'] = keys
