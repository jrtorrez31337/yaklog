#!/usr/bin/env bash
# deploy-from-bundle.sh — deploy the latest Plexus install bundle to one
...
#   demo      — remote demo VM (SSH: plexus-admin@$DEMO_HOST)
#   both      — devel then demo (fail-fast: demo skipped if devel fails)
#
# Env overrides:
#   DEMO_HOST         — default 10.71.1.184
#   DEMO_INSTALL_DIR  — default /opt/plexus-demo
#   DEMO_USER         — default plexus-admin
...
DEMO_USER="${DEMO_USER:-plexus-admin}"
DEMO_INSTALL_DIR="${DEMO_INSTALL_DIR:-/opt/plexus-demo}"
...
if [[ -z "$BUNDLE_DIR" ]]; then
  DIST="$REPO_ROOT/dist"
  BUNDLE_DIR=$(ls -td "$DIST"/plexus-install-bundle-*/ 2>/dev/null | head -1 | sed 's|/$||')
  if [[ -z "$BUNDLE_DIR" ]]; then
    err "no bundle found in $DIST. Run plexus-install/build-bundle.sh first."
    exit 3
  fi
fi
