#!/usr/bin/env bash
# uninstall.sh — tear down a Plexus instance.
# Task #284 / PLAN-PLEXUS-INSTALL-BUNDLE.md.
#
...
By default this is a DRY-RUN. Pass --force to actually delete.
  ...
  --install-dir <path>  Plexus install dir (default: /opt/plexus)
...
echo "=== Uninstalling Plexus '$INSTANCE_NAME' from $INSTALL_DIR ==="
...
if command -v systemctl >/dev/null && [[ "$(id -u)" == "0" || -n "${SUDO_UID:-}" ]]; then
  ...
  run rm -rf "/etc/plexus-${INSTANCE_NAME}"
  run rm -rf "/var/lib/plexus-${INSTANCE_NAME}"
