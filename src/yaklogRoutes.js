// Yaklog query proxy — Plan C Stage 2 CP1.
//
...
//
// Stage 2 V1 surface:
//   GET /api/v1/yaklog/templates                     — discovery; lists allowlist
//   GET /api/v1/yaklog/query?template=X&...          — instant query
//   GET /api/v1/yaklog/query_range?template=X&...    — range query (step + from + to)
//
...
const templates = {
  ...
  'tokens.rate.byAgent': {
    ...
    build({ window }) {
      ...
      return `sum by (yaklog_agent_id, model, type) (rate(claude_code_token_usage_tokens_total[${window}]))`;
    },
  },
  ...
  'cost.rate.byAgent': {
    ...
    build({ window }) {
      requireEnum('window', window, RATE_WINDOW_ALLOWLIST);
      return `sum by (yaklog_agent_id, model) (rate(claude_code_cost_usage_USD_total[${window}]))`;
    },
  },
  ...
  'session.count.byAgent': {
    ...
    params: {},
    build() {
      return 'sum by (yaklog_agent_id) (claude_code_session_count_total)';
    },
  },
  ...
  'active_time.rate.byAgent': {
    ...
    build({ window }) {
      requireEnum('window', window, RATE_WINDOW_ALLOWLIST);
      return `sum by (yaklog_agent_id) (rate(claude_code_active_time_seconds_total[${window}]))`;
    },
  },
  ...
  // host_arch, os_*, terminal_type, service_version, yaklog.*) but
  ...
  'agent.identity.byAgentId': {
    ...
    build({ agent_id }) {
      ...
      return `last_over_time(claude_code_active_time_seconds_total{yaklog_agent_id="${id}"}[24h]) or last_over_time(gemini_runtime_cycle_total{yaklog_agent_id="${id}"}[24h])`;
    },
  },
  ...
  'cluster.cost.topAgents': {
    ...
    build() {
      ...
      return 'topk(10, sum by (yaklog_agent_id) (last_over_time(claude_code_cost_usage_USD_total{yaklog_agent_id!~"yaklog-admin|jon|Jon|ops|admin"}[24h])))';
    },
  },
  ...
  'cluster.cost.topAgentTokens': {
    ...
    build() {
      ...
      // emit yaklog_tokens_*_total but no USD counter (canonical CP11.x.1
      ...
      return 'topk(10, sum by (yaklog_agent_id) (last_over_time(yaklog_tokens_input_total{yaklog_agent_id!~"yaklog-admin|jon|Jon|ops|admin"}[24h]) + last_over_time(yaklog_tokens_output_total{yaklog_agent_id!~"yaklog-admin|jon|Jon|ops|admin"}[24h])))';
    },
  },
...
function cacheSet(key, body) {
  if (cache.size >= config.yaklogQueryCacheMaxEntries) {
    // Evict oldest insert (Map preserves insertion order)
    const firstKey = cache.keys().next().value;
    cache.delete(firstKey);
  }
  cache.set(key, { expiresAt: Date.now() + config.yaklogQueryCacheTtlMs, body });
}
...
async function promFetch(path, params) {
  ...
  const t = setTimeout(() => controller.abort(), config.yaklogQueryTimeoutMs);
  try {
    ...
    if (err.name === 'AbortError') {
      return { httpStatus: 504, body: { status: 'error', errorType: 'timeout', error: `Prom did not respond within ${config.yaklogQueryTimeoutMs}ms` } };
    }
...
// Mounted at /api/v1/yaklog/public/ in app.js.
//
...
// See src/yaklogStreamer.js for the architecture rationale.
const { streamHandler: yaklogStreamHandler } = require('./yaklogStreamer');
publicRouter.get('/stream', yaklogStreamHandler);
...
publicRouter.get('/cost/export', (req, res) => {
  ...
  const period = String(req.query.period || 'mtd');
  const schema = String(req.query.schema || 'yaklog-default');
  if (format !== 'csv') {
  ...
  res.setHeader('Content-Disposition', `attachment; filename="yaklog-cost-${period}-${range.from}-to-${range.to}-${schema}.csv"`);
...
// per the rest of /api/v1/yaklog/public/*). Returns one row per channel
...
// the rest of /api/v1/yaklog/public/*; proper browser-auth comes Stage 2.5+).
//
...
// /api/v1/yaklog/public/*). Returns registration rows from the ADR-0025
