// Plan C Stage 2.5 — Yaklog SSE streamer.
//
...
// Architecture:
//
//   Browser opens EventSource('/api/v1/yaklog/public/stream')
...
//   - the request-driven /api/v1/yaklog/query for on-demand queries (e.g. popover)
...
// Mirrors the templates in yaklogRoutes.js but with concrete params bound,
...
const FRAMES = [
  ...
  },
  {
    ...
    lookbackS: 3600,
    step: '15s',
    promql: 'sum by (yaklog_agent_id, model) (rate(claude_code_cost_usage_USD_total[5m]))',
  },
  {
    ...
    lookbackS: 3600,
    step: '15s',
    promql: 'sum by (yaklog_agent_id, model, type) (rate(claude_code_token_usage_tokens_total[5m]))',
  },
  ...
  },
  {
    ...
    // Excludes ops/relay senders (yaklog-admin/jon/Jon) that never had a real
    ...
    promql: 'topk(10, sum by (yaklog_agent_id) (last_over_time(claude_code_cost_usage_USD_total{yaklog_agent_id!~"yaklog-admin|jon|Jon|ops|admin"}[24h])))',
  },
  {
    // Top-10 token-spend drivers by agent (multi-runtime; catches codex/gemini
    // agents that emit yaklog_tokens_*_total but no USD counter per canonical
    ...
    kind: 'instant',
    promql: 'topk(10, sum by (yaklog_agent_id) (last_over_time(yaklog_tokens_input_total{yaklog_agent_id!~"yaklog-admin|jon|Jon|ops|admin"}[24h]) + last_over_time(yaklog_tokens_output_total{yaklog_agent_id!~"yaklog-admin|jon|Jon|ops|admin"}[24h])))',
  },
  {
  ...
  },
  {
    ...
    lookbackS: 3600,
    step: '15s',
    promql: 'sum by (yaklog_agent_id) (rate(claude_code_active_time_seconds_total[5m]))',
  },
];
...
class YaklogStreamer extends EventEmitter {
  constructor() {
    super();
  ...
  start() {
    ...
    console.log(`[yaklog-streamer] started: ${FRAMES.length} frames, interval=${POLL_INTERVAL_MS}ms`);
  }
  ...
  _kickFramePoll(frame) {
    ...
    this._pollFrame(frame).catch((e) => {
      console.warn(`[yaklog-streamer] initial poll failed for ${frame.name}:`, e.message);
    });
    const timer = setInterval(() => {
      this._pollFrame(frame).catch((e) => {
        console.warn(`[yaklog-streamer] poll failed for ${frame.name}:`, e.message);
      });
    }, POLL_INTERVAL_MS);
  ...
  async _pollFrame(frame) {
    ...
    const url = new URL(`${config.yaklogPromUrl}/api/v1/${isInstant ? 'query' : 'query_range'}`);
    ...
    const timeout = setTimeout(() => controller.abort(), config.yaklogQueryTimeoutMs);
    let body;
    try {
      ...
      console.warn(`[yaklog-streamer] ${frame.name}: ${e.message}`);
      return;
    } finally {
...
const streamer = new YaklogStreamer();

// ──────────────────────────────────────────────────────────────────────
// SSE handler — mounted at /api/v1/yaklog/public/stream (no auth; mirrors
...
module.exports = {
  ...
  streamHandler,
  FRAMES,
  _internals: { hashFrame, YaklogStreamer },
};
