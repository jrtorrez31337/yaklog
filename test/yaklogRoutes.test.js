// Plan C Stage 2 CP1 — yaklogRoutes proxy tests.
//
...
// section at bottom of yaklogRoutes.js / Stage 2 build plan).
...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'yaklog-test-yaklog-'));
process.env.YAKLOG_DB_PATH = path.join(tempDir, 'yaklog.db');
process.env.YAKLOG_API_KEYS = 'test-key';
process.env.YAKLOG_YAKLOG_PROM_URL = 'http://prom-stub.invalid:9090';
process.env.YAKLOG_YAKLOG_QUERY_CACHE_TTL_MS = '60000';
process.env.YAKLOG_YAKLOG_QUERY_TIMEOUT_MS = '500';
process.env.NODE_ENV = 'test';

// Stub global fetch BEFORE loading the app, since yaklogRoutes captures
...
const { _internals } = require('../src/yaklogRoutes');
...
test('yaklog routes require Bearer (unauth → 401)', async () => {
  const r = await request(app).get('/api/v1/yaklog/templates');
  assert.equal(r.statusCode, 401);
});

test('yaklog routes accept valid Bearer', async () => {
  const r = await request(app).get('/api/v1/yaklog/templates').set(authed);
...
test('arbitrary PromQL (no template param) → 400', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/query?expr=sum(rate(node_cpu[5m]))')
    .set(authed);
...
test('unknown template name → 422', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/query?template=not.a.real.template')
    .set(authed);
...
test('range-only template via /query → 422', async () => {
  ...
  try {
    const r = await request(app).get(`/api/v1/yaklog/query?template=${tmplName}`).set(authed);
    assert.equal(r.statusCode, 422);
...
test('instant-only template via /query_range → 422', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/query_range?template=agent.identity.byAgentId&agent_id=foo&from=1779000000&to=1779001000&step=15s')
    .set(authed);
  assert.equal(r.statusCode, 422);
...
test('missing required param → 400', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/query?template=cost.cumulative.byDim')
    .set(authed);
  assert.equal(r.statusCode, 400);
...
test('param not in enum → 400 with allowed list', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/query?template=cost.cumulative.byDim&dim=hostname')
    .set(authed);
  assert.equal(r.statusCode, 400);
...
test('agent_id with unsafe chars (PromQL escape attempt) → 400', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/query?template=agent.identity.byAgentId&agent_id=foo"bar')
    .set(authed);
  assert.equal(r.statusCode, 400);
...
test('agent_id with valid shape passes validation', async () => {
  ...
  const r = await request(app)
    .get('/api/v1/yaklog/query?template=agent.identity.byAgentId&agent_id=parch-agent')
    .set(authed);
  assert.equal(r.statusCode, 200);
  assert.equal(r.body.template, 'agent.identity.byAgentId');
  assert.equal(r.body.params.agent_id, 'parch-agent');
  assert.match(r.body.query, /yaklog_agent_id="parch-agent"/);
});

test('window param outside allowlist → 400 (1y window attack)', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/query?template=tokens.rate.byAgent&window=1y')
    .set(authed);
  assert.equal(r.statusCode, 400);
...
test('window param default applied when omitted', async () => {
  nextFetchResponse = { status: 200, body: { status: 'success', data: { result: [] } } };
  const r = await request(app)
    .get('/api/v1/yaklog/query?template=tokens.rate.byAgent')
    .set(authed);
  assert.equal(r.statusCode, 200);
...
test('range query missing from/to/step → 400', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/query_range?template=tokens.rate.byAgent')
    .set(authed);
  assert.equal(r.statusCode, 400);
...
test('range query with bad step → 400', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/query_range?template=tokens.rate.byAgent&from=1779000000&to=1779001000&step=1y')
    .set(authed);
  assert.equal(r.statusCode, 400);
...
test('range query with malformed from → 400', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/query_range?template=tokens.rate.byAgent&from=not-a-time&to=1779001000&step=15s')
    .set(authed);
  assert.equal(r.statusCode, 400);
...
test('valid range query reaches Prom + returns range echo', async () => {
  ...
  const r = await request(app)
    .get('/api/v1/yaklog/query_range?template=tokens.rate.byAgent&from=1779000000&to=1779001000&step=15s')
    .set(authed);
  assert.equal(r.statusCode, 200);
...
test('repeat instant query → second hit returns cache (X-Yaklog-Cache: hit)', async () => {
  nextFetchResponse = { status: 200, body: { status: 'success', data: { result: [{ metric: { yaklog_agent_id: 'a' }, value: [1, '5'] }] } } };
  const r1 = await request(app)
    .get('/api/v1/yaklog/query?template=session.count.byAgent')
    .set(authed);
  assert.equal(r1.statusCode, 200);
  assert.equal(r1.headers['x-yaklog-cache'], 'miss');

  const r2 = await request(app)
    .get('/api/v1/yaklog/query?template=session.count.byAgent')
    .set(authed);
  assert.equal(r2.statusCode, 200);
  assert.equal(r2.headers['x-yaklog-cache'], 'hit');
...
test('different params produce distinct cache keys', async () => {
  nextFetchResponse = { status: 200, body: { status: 'success', data: { result: [] } } };
  await request(app).get('/api/v1/yaklog/query?template=cost.cumulative.byDim&dim=model').set(authed);
  nextFetchResponse = { status: 200, body: { status: 'success', data: { result: [] } } };
  await request(app).get('/api/v1/yaklog/query?template=cost.cumulative.byDim&dim=user_email').set(authed);
...
test('/yaklog/public/query accepts request WITHOUT Bearer', async () => {
  nextFetchResponse = { status: 200, body: { status: 'success', data: { result: [] } } };
  const r = await request(app)
    .get('/api/v1/yaklog/public/query?template=session.count.byAgent');
    // NO .set(authed) — public surface
  assert.equal(r.statusCode, 200);
  assert.equal(r.body.template, 'session.count.byAgent');
});

test('/yaklog/public/query_range works without Bearer', async () => {
  nextFetchResponse = { status: 200, body: { status: 'success', data: { resultType: 'matrix', result: [] } } };
  const r = await request(app)
    .get('/api/v1/yaklog/public/query_range?template=tokens.rate.byAgent&from=1779000000&to=1779001000&step=15s');
  assert.equal(r.statusCode, 200);
  assert.equal(r.body.range.step, '15s');
});

test('/yaklog/public/templates exposes the allowlist (browser discovery)', async () => {
  const r = await request(app).get('/api/v1/yaklog/public/templates');
  assert.equal(r.statusCode, 200);
  assert.ok(r.body.templates['tokens.rate.byAgent']);
});

test('/yaklog/public enforces SAME allowlist as auth surface (no bypass)', async () => {
  // Verify the public surface can't escape the allowlist (no auth ≠ no validation)
  const r = await request(app)
    .get('/api/v1/yaklog/public/query?template=cost.cumulative.byDim&dim=password');
  assert.equal(r.statusCode, 400);
  assert.match(r.body.error, /dim must be one of/);
});

test('/yaklog/public enforces SAME unknown-template handling', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/public/query?template=arbitrary_promql_attempt');
  assert.equal(r.statusCode, 422);
});

test('/yaklog auth-required path still rejects unauth even though /public exists', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/query?template=session.count.byAgent');
...
test('CP4: agent_id with PromQL escape attempt (newline) → 400', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/public/query?template=agent.identity.byAgentId&agent_id=' + encodeURIComponent('foo\nbar'))
    ;
  assert.equal(r.statusCode, 400);
});
...
test('CP4: agent_id with PromQL escape attempt (backslash) → 400', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/public/query?template=agent.identity.byAgentId&agent_id=' + encodeURIComponent('foo\\bar'))
    ;
  assert.equal(r.statusCode, 400);
});
...
test('CP4: agent_id at the 128-char boundary passes', async () => {
  ...
  const r = await request(app)
    .get('/api/v1/yaklog/public/query?template=agent.identity.byAgentId&agent_id=' + encodeURIComponent(longButValidId));
  assert.equal(r.statusCode, 200);
});
...
test('CP4: agent_id one over the 128-char boundary → 400', async () => {
  ...
  const r = await request(app)
    .get('/api/v1/yaklog/public/query?template=agent.identity.byAgentId&agent_id=' + encodeURIComponent(tooLong));
  assert.equal(r.statusCode, 400);
});
...
test('CP4: agent_id with allowed special chars (@.+:/-_) passes', async () => {
  nextFetchResponse = { status: 200, body: { status: 'success', data: { result: [] } } };
  const r = await request(app)
    .get('/api/v1/yaklog/public/query?template=agent.identity.byAgentId&agent_id=' + encodeURIComponent('email@host.com/path-to:res_v1'));
  assert.equal(r.statusCode, 200);
});
...
test('CP4: time params with negative numbers reach Prom (Prom rejects upstream)', async () => {
  ...
  const r = await request(app)
    .get('/api/v1/yaklog/public/query_range?template=tokens.rate.byAgent&from=-1&to=0&step=15s');
...
test('CP4: step whitespace rejected (`15 s` !== `15s`)', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/public/query_range?template=tokens.rate.byAgent&from=1779000000&to=1779001000&step=' + encodeURIComponent('15 s'));
  assert.equal(r.statusCode, 400);
});

test('CP4: empty template name → 400 (not 422)', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/public/query?template=');
  assert.equal(r.statusCode, 400);
  assert.match(r.body.error, /missing param: template/);
});
...
test('CP4: dim with PromQL-special chars (curly brace) → 400 via enum mismatch', async () => {
  const r = await request(app)
    .get('/api/v1/yaklog/public/query?template=cost.cumulative.byDim&dim=' + encodeURIComponent('user_email{'));
  assert.equal(r.statusCode, 400);
});
...
test('CP4: window with shell-injection attempt → 400', async () => {
  ...
  const r = await request(app)
    .get('/api/v1/yaklog/public/query?template=tokens.rate.byAgent&window=' + encodeURIComponent('5m;DROP'));
  assert.equal(r.statusCode, 400);
});
